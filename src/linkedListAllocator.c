...
heapBlock *initialHeapBlock = NULL; 
...
heapBlock* blockSplitter(heapBlock *block, size_t size){
 ...
 if((int)block->size - sizeof(heapBlock) - size >= 1) { // check if we can afford to split the block into 2 and the 2nd block is actually usable, ensure that the size isn't uint
 ...
 block->free = false;
 }else{ // if not, then return the whole block without splitting
   return block; 
 }
 return block;
}
...
void* malloc_internal(size_t size){
      if(size == 0){
        return NULL;
      }else if(initialHeapBlock != (heapBlock*)&_heap_start){
